// Doodle decorations — minimalist hand-drawn SVG ornaments
// that float around images. Tied to --sk-show-doodles so the tweak
// "Show annotations" toggles them globally.

const DOODLE_COLOR = SK ? SK.accent : 'var(--sk-accent)';
...
const Doodles = {
  ...
  circle: ({ size = 40, color, stroke = 2 }) => (
    <svg width={size} height={size} viewBox="0 0 100 100" fill="none" style={{ overflow: 'visible' }}>
      <path d="M50 6 C 76 8 94 26 92 52 C 90 78 70 92 46 90 C 22 88 6 70 8 44 C 10 22 28 6 52 8" stroke={color || DOODLE_COLOR} strokeWidth={stroke} strokeLinecap="round" fill="none" />
    </svg>
  ),
  ...
  dots: ({ size = 30, color }) => (
    <svg width={size} height={size} viewBox="0 0 30 30" fill={color || DOODLE_COLOR}>
      <circle cx="5" cy="15" r="2" /><circle cx="15" cy="15" r="2.2" /><circle cx="25" cy="15" r="2" />
    </svg>
  ),
  ...
  rays: ({ size = 42, color }) => (
    <svg width={size} height={size} viewBox="0 0 42 42" fill="none">
      {Array.from({ length: 8 }).map((_, i) => {
        ...
        const x1 = 21 + Math.cos(a) * 10, y1 = 21 + Math.sin(a) * 10;
        const x2 = 21 + Math.cos(a) * 18, y2 = 21 + Math.sin(a) * 18;
...
// Place inside a position:relative parent. Respects --sk-show-doodles.
const Doodle = ({ kind = 'sparkle', pos = 'tr', size, color, offset = 8, rotate = 0, opacity = 1 }) => {
...
// Renders as a transparent block-level wrapper that lets doodles peek outside.
const Dood = ({ children, dood = [], style = {}, display = 'block' }) => (
  <div className="sk-dood-wrap" style={{ position: 'relative', display, width: '100%', height: style.height || 'auto', ...style }}>
    {children}
    {dood.map((d, i) => <Doodle key={i} {...d} />)}
  </div>
);

Object.assign(window, { Doodle, Doodles, Dood });
