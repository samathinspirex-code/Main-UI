...
export default function AcademicAdmissions({ programs, programmes, initialProgramSlug = "" }: Props) {
  ...
  async function submit(){if(!stepOneValid||!stepTwoValid)return;setBusy(true);setError("");try{const result_document=document?{filename:document.name,content_type:document.type,size_bytes:document.size,data_base64:await fileToBase64(document)}:null;const response=await fetch("/api/programme-enrolments",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({submission_id:submissionId,...details,highest_qualification:qualification,programme_id:Number(programmeId),preferred_course_id:Number(courseId),preferred_study_mode:selectedStudyOption?.study_mode??studyMode,result_document})});const payload=await response.json();if(!response.ok)throw new Error(payload?.error?.message||payload?.detail||"Unable to submit your application.");localStorage.removeItem(DRAFT_KEY);setComplete(true);}catch(reason){setError(reason instanceof Error?reason.message:"Unable to submit your application.");}finally{setBusy(false);}}
