...
type Draft = Details & { programmeId: string; courseId: string; qualification: string; step: number };
const STEPS = [["01","Your details","Name, email & phone"],["02","Pick your course","Programme, then course"],["03","Upload results","Optional documents"],["04","Review & submit","Send to admissions"]];
...
export default function AcademicAdmissions({ programs, programmes, initialProgramSlug = "" }: Props) {
  ...
  function saveDraft(exit=false){localStorage.setItem(DRAFT_KEY,JSON.stringify({...details,programmeId,courseId,qualification,step} satisfies Draft));setSaved(true);window.setTimeout(()=>setSaved(false),2500);if(exit)router.push("/");}
  ...
  return <div><Navbar/><main className="admissions-page">
    <header className="admission-hero sx"><Tag accent>START YOUR APPLICATION</Tag><h1>Apply in four<br/><span>simple steps.</span></h1><p>Tell us what you would like to study. Save your progress, return later, and an advisor will guide you from here.</p></header>
    ...
    <section className="admission-layout sx"><div className="admission-card">
      ...
      {step===2&&<div className="admission-panel"><PanelHead step="STEP 02" title="Pick your course" text="Choose a programme first. We only show courses inside that programme."/><div className="admission-fields"><label>Highest qualification<select value={qualification} onChange={e=>setQualification(e.target.value)}>{QUALIFICATIONS.map(item=><option key={item}>{item}</option>)}</select></label><label>Programme<select required value={programmeId} onChange={e=>{setProgrammeId(e.target.value);setCourseId("")}}><option value="">Choose a programme</option>{programmeOptions.map(item=><option value={item.programme_id} key={item.programme_id}>{item.name}</option>)}</select><small className="field-help">Programmes are alphabetical; long lists scroll inside the selector.</small></label>{programmeId&&<label>Course<select required value={courseId} onChange={e=>setCourseId(e.target.value)}><option value="">Choose a course in {selectedProgramme?.name}</option>{courseOptions.map(item=><option value={item.courseId} key={item.courseId}>{item.title}</option>)}</select></label>}{selectedCourse&&<div className="admission-selection"><Icon kind="star" size={20} color="var(--accent)"/><div><b>{selectedCourse.title}</b><span>{selectedCourse.code} · {selectedCourse.duration} · {selectedCourse.schoolName??selectedCourse.school}</span></div></div>}</div></div>}
      ...
      {step===4&&<div className="admission-panel"><PanelHead step="STEP 04" title="Review & submit" text="Check your details. Submitting creates a lead for our admissions team."/><dl className="admission-review">{[["Name",details.full_name],["Email",details.email],["Phone",details.phone],["Qualification",qualification],["Programme",selectedProgramme?.name||"—"],["Course",selectedCourse?.title||"—"],["Result document",document?.name||"Not uploaded — provide later"]].map(([key,value])=><div key={key}><dt>{key}</dt><dd>{value}</dd></div>)}</dl><div className="admission-consent"><Icon kind="check" size={18} color="var(--accent)"/><span>Your application will appear in the CRM as a new admissions lead for follow-up.</span></div></div>}
