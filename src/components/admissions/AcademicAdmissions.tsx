...
export default function AcademicAdmissions({ programs, programmes, initialProgramSlug = "" }: Props) {
  ...
  const courses = programmeCourses;
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/programme-enrolments", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
        ...details, programme_id: Number(programmeId),
  ...
  return <div><Navbar /><main style={{ paddingTop: 68, minHeight: "75vh" }}>
    <section className="sx" style={{ maxWidth: 1000, margin: "0 auto", padding: "64px 0 90px" }}>
      ...
      <p style={{ color: "var(--ink-soft)", fontSize: 17, lineHeight: 1.65, maxWidth: 720 }}>Start with the school and broad programme you are interested in. Course and study mode can still be decided with your counsellor.</p>
      ...
      <form onSubmit={submit} style={{ marginTop: 38, padding: 34, border: "1px solid var(--border)", borderRadius: 14, background: "var(--surface)", display: "grid", gap: 22 }}>
        ...
        <div><label style={label}>SCHOOL *</label><select required style={field} value={schoolId} onChange={e => { setSchoolId(e.target.value); setProgrammeId(""); setCourseId(""); }}><option value="">Choose a school</option>{schools.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select></div>
        {schoolId && <div><label style={label}>PROGRAMME *</label><select required style={field} value={programmeId} onChange={e => { setProgrammeId(e.target.value); setCourseId(""); }}><option value="">Choose Foundation, HND, Postgraduate…</option>{availableProgrammes.map(item => <option key={item.programme_id} value={item.programme_id}>{item.name}</option>)}</select></div>}
